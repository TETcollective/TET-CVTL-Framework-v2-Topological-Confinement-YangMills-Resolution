...
\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{eternal_braider_torque_visualization.jpeg}
...
2. **Calcolo del proxy del vacuum torque**  
   ...
   \end{equation}
   dove $\mathbf{v} = d\mathbf{r}/dt$ (normalizzato in post-processing per coerenza con eq.~\eqref{eq:vbraid_normalized}). Il risultato numerico, perfettamente coerente con la derivazione analitica, è
   \begin{equation}
...
Il calcolo è implementato nella funzione dedicata \texttt{compute\_tau\_vac\_proxy()} (file \texttt{code/compute\_tau\_vac\_proxy\_normalized.py}) e si articola in due fasi principali:
...
Le simulazioni numeriche RK45 (file \texttt{eternal\_braider\_simulation.py} e \texttt{tau\_vac\_proxy\_from\_trajectory.py} in appendice) confermano che:
...
L’integrazione è stata eseguita con il metodo Runge-Kutta-Fehlberg di ordine 4(5) adattivo (\texttt{RK45}) implementato in \texttt{scipy.integrate.solve_ivp}, scelto per:
- ordine locale 5 con controllo di errore globale di ordine 4,
- adattività del passo per gestire la non-linearità della forzatura chirale,
- elevata stabilità per sistemi non rigidi.

Parametri utilizzati:
\begin{itemize}
\item tolleranza relativa \texttt{rtol = 1e-8},
\item tolleranza assoluta \texttt{atol = 1e-10},
\item passo massimo \texttt{max_step = 0.1},
\item output denso (\texttt{dense_output=True}) per interpolazione accurata.
\end{itemize}

Condizioni iniziali di test (leggermente fuori dal ciclo ideale):
\begin{equation}
\mathbf{y}(0) = [0.5,\ 0.0,\ 0.2]^\top.
\label{eq:initial_condition}
\end{equation}

Intervallo di integrazione: \(t \in [0, 100]\) (circa 2.65 periodi completi), con 2000 punti di valutazione equispaziati.
...
I test numerici di robustezza (file \texttt{code/test\_convergence\_multi\_run.py}) mostrano che perturbazioni casuali non destabilizzano il trifoglio Nash-stabile, confermando la protezione topologica del meccanismo. Tuttavia, la realizzabilità pratica di TET--CVTL-lite dipende ancora da:
...
\begin{table}[H]
\centering
\small % oppure \footnotesize se vuoi ancora più piccolo
\caption{Riassunto delle applicazioni pratiche del TET--CVTL-lite}
\label{tab:applications}
\renewcommand{\arraystretch}{1.3} % spazio verticale tra righe
\setlength{\tabcolsep}{4pt} % riduce spazio tra colonne
\resizebox{\textwidth}{!}{%
